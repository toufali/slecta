...
const context = () => ({
  query: {}, params: { id: 7 }, headers: {}, cookies: { get: () => undefined },
...
test('a cached score is served without touching TMDB', async () => {
  ...
  assert.deepEqual(ctx.body, { scores: { imdb: 81 }, counts: { imdb: 500_000 }, avgScore: 91, lowConfidence: false },
...
test('the detail page marks a thin score and leaves a settled one alone', async () => {
  ...
  for (const [record, marked] of [[thin, true], [settled, false], [scoreless, false]]) {
    ...
    assert.equal(/<score-badge[^>]* low-confidence[ >]/.test(ctx.body), marked, JSON.stringify(record))
    assert.equal(/<li class='disclaimer low-confidence'>/.test(ctx.body), marked, 'the line follows the colour')
  }
})
