...
// Bucketed by the last 3 characters of the id: a lookup reads one ~10KB string, not a 400k-field hash Redis cannot store compactly
...
class ImdbService {
  async refresh() {
    ...
    // Chunked because node-redis pipelines concurrent commands.
    // setCache hides Redis errors, so unchecked writes would report a refresh that never landed.
